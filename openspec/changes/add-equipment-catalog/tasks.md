...
- [x] 2.3 Implement explicit catalog-template reapplication and editable prefilling without silently changing configured equipment.
- [x] 2.4 Add cross-engine repository and migration tests for legacy/manual equipment, string module composition and derived capacity, contradictory totals, overflow/bounds errors, unchanged and edited prefills, optional provenance, and catalog upgrades.
...
- [x] 3.2 Extend inverter/string equipment write and read contracts with optional template references, confirmed snapshot data, module count, manufacturer/model, per-module and derived total peak power, manual/copied/customized provenance, and safe value-validation errors.
- [x] 3.3 Document catalog schemas, normalized units, pagination, authentication, examples, optional prefilling, editable values, and snapshot semantics in OpenAPI.
- [x] 3.4 Add API authorization, schema, filtering, not-found, edited-prefill, manual-entry, and bidirectional route-coverage tests.
...
- [x] 5.3 Run warning-free Rust checks/tests, frontend lint/typecheck/tests/build, OpenAPI lint/compare/coverage, migration profiles, and production embedded-UI validation.
