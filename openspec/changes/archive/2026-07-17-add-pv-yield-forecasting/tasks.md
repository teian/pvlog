...
- [x] 2.4 Implement targeted invalidation and retention operations that preserve referenced issued forecasts and historical performance inputs.
- [x] 2.5 Add SQLite/PostgreSQL migration and repository tests for legacy systems, round trips, immutable revisions, stale selection, effective boundaries, idempotent writes, partial capacity, invalidation, and retention.
...
- [x] 3.5 Add provider contract tests for valid normalization, malformed/overlapping intervals, unsupported units, missing irradiance, revisions, stale fallback, open circuits, location mismatch, and provider-independent domain output.
...
- [x] 5.5 Add worker integration tests for duplicate jobs, retries, provider outage, stale results, intersecting invalidations, correction-driven rebuilds, configuration boundaries, and telemetry independence.
...
- [x] 6.6 Add API/contract tests for authorization, bounds, ETags, effective settings, run selection, stale/unavailable providers, partial aggregation, model boundaries, exports, generic failures, and continued ingestion availability.
...
- [x] 7.2 Add localized forecast-input completeness and effective-dated loss/calibration administration linked to existing system and string configuration.
...
- [x] 8.3 Add deterministic backfill/recalculation commands with dry-run, bounded account/range selection, progress, cancellation/retry, and rollback guidance.
- [x] 8.4 Run warning-free Rust checks and focused tests, SQLite/PostgreSQL profiles, frontend lint/typecheck/tests/build, Playwright, OpenAPI lint/compare/coverage, provider/model fixtures, security checks, and production embedded-UI validation.
